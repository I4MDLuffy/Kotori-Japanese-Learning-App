    ...
    Scaffold(
        ...
        Column(
            ...
            OutlinedTextField(
                ...
                onValueChange = { vm.dispatchAction(VerbListAction.Search(it)) },
                modifier = Modifier
                    .fillMaxWidth()
                ...
                trailingIcon = {
                    if (state.searchQuery.isNotBlank()) {
                        IconButton(onClick = { vm.dispatchAction(VerbListAction.Search("")) }) {
                    ...
                    state.availableJlptLevels.forEach { level ->
                        FilterChip(
                            ...
                            onClick = {
                                ...
                                vm.dispatchAction(VerbListAction.FilterByJlpt(next))
                            },
                            label = { Text(level) },
                        )
                    }
            ...
            Text(
                text = "${state.displayedEntries.size} verbs",
            ...
            if (state.isLoading) {
                ...
                Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
                    Text("No verbs found", color = MaterialTheme.colorScheme.onSurfaceVariant)
                }
            } else {
...
private fun VerbListItem(entry: VerbEntry, onClick: () -> Unit) {
    ...
    Row(
        ...
        Column(modifier = Modifier.weight(1f)) {
            ...
            if (entry.dictionaryForm.isNotBlank() && entry.dictionaryForm != entry.kanji) {
                Text(
        ...
        Column(horizontalAlignment = Alignment.End) {
            ...
            Row(
                ...
                if (entry.verbType.isNotBlank()) {
                    SuggestionChip(
                        onClick = {},
                        label = { Text(entry.verbType, fontSize = 11.sp) },
